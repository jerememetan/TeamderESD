...
class PeerEvalRound(db.Model):
    ...
    round_id = db.Column(PG_UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    section_id = db.Column(PG_UUID(as_uuid=True), nullable=False)
...
class PeerEvalSubmission(db.Model):
    ...
    submission_id = db.Column(PG_UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    round_id = db.Column(PG_UUID(as_uuid=True), db.ForeignKey("peer_eval_rounds.round_id", ondelete="CASCADE"), nullable=False)
    evaluator_id = db.Column(db.Integer, nullable=False)
    evaluatee_id = db.Column(db.Integer, nullable=False)
    team_id = db.Column(PG_UUID(as_uuid=True), nullable=False)
