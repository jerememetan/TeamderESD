...
# Optional marshmallow support: allow view functions to expose marshmallow Schemas
try:
    import marshmallow
...
def _build_openapi_spec(app: Flask, metadata: ServiceDocMetadata) -> Dict[str, Any]:
    ...
    for rule in app.url_map.iter_rules():
        ...
        for method in methods:
            ...
            # Merge explicit OpenAPI metadata from the view function if present.
            view_func = app.view_functions.get(rule.endpoint)
            if view_func is not None:
                explicit = getattr(view_func, "__openapi__", None)
                if explicit:
                    # Merge parameters (explicit overrides or adds)
                    if "parameters" in explicit:
                        operation["parameters"] = explicit["parameters"]
                    # Merge requestBody
                    if "requestBody" in explicit:
                        operation["requestBody"] = explicit["requestBody"]
                    # Merge responses (extend defaults)
                    if "responses" in explicit:
                        # ensure responses dict exists
                        operation.setdefault("responses", {})
                        for code, resp in explicit["responses"].items():
                            operation["responses"][code] = resp
                    # Override summary/description if provided
                    if "summary" in explicit:
                        operation["summary"] = explicit["summary"]
                    if "description" in explicit:
                        operation["description"] = explicit["description"]
