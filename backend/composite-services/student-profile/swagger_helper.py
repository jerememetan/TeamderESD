    ...
    for rule in app.url_map.iter_rules():
        ...
        for method in methods:
            ...
            if view_fn is not None:
                ...
                # request schema for mutating methods
...
def _convert_marshmallow_schema(schema_cls_or_instance) -> Dict[str, Any]:
    """Convert a Marshmallow Schema class or instance to an OpenAPI schema object.

    This function imports marshmallow at runtime so the project doesn't hard-depend
    on marshmallow unless it's present in the service.
    """
    try:
        marshmallow = importlib.import_module("marshmallow")
    except Exception:
        # marshmallow not available; return a generic object schema
        return {"type": "object"}

    Schema = getattr(marshmallow, "Schema", None)
