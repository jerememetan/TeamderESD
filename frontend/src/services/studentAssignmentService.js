...
function normalizeSection(section, index) {
  return {
    ...
    sectionNumber: Number(section?.section_number ?? section?.sectionNumber ?? index + 1),
...
export async function fetchStudentAssignments({ studentProfile }) {
  ...
  if (failure) {
    throw failure.reason instanceof Error
      ? failure.reason
      : new Error('Unable to connect to the backend team or student profile service.');
  }
  ...
  for (const result of sectionResults) {
    ...
    assignments.push({
      ...
      groupId: section.id,
      groupCode: course?.code ? `${course.code}G${section.sectionNumber}` : section.id,
