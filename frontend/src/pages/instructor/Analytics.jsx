...
function Analytics() {
  ...
  const yearDistributionData = useMemo(() => {
    const counts = backendStudents.reduce((accumulator, student) => {
      const key = student?.profile?.year ? `Year ${student.profile.year}` : "Unknown";
    ...
    <div className={styles.page}>
      ...
      <section className={styles.hero}>
        <div>
          ...
          <p className={styles.subtitle}>{selectedCourse.name} :: compare this group against sibling clusters while using live roster totals from student-profile where available.</p>
        </div>
        <div className={styles.heroTags}>
          <GroupChip code={selectedGroup.code} meta={`${totalStudents} students · ${groupTeams.length} teams`} tone="green" />
          <SystemTag tone={rosterSourceTone}>{isLoadingRoster ? 'Loading roster' : backendStudents.length ? 'Live roster loaded' : 'Roster fallback'}</SystemTag>
        </div>
      </section>
      ...
      <section className={styles.statsGrid}>
        ...
        <ModuleBlock componentId="MOD-A3" eyebrow="Roster" title="Live Section Students" metric={String(totalStudents).padStart(2, "0")} metricLabel={backendStudents.length ? 'Pulled from student-profile' : 'Using frontend fallback count'} accent="orange" />
      </section>
      ...
      <section className={styles.chartGrid}>
        ...
        <ModuleBlock componentId="MOD-A8" eyebrow="Roster Mix" title="Year Distribution" className={`${styles.chartModule} ${styles.fullSpan}`}>
          <div className={styles.chartWrap}>
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={yearDistributionData.length ? yearDistributionData : responseRateData}>
                <CartesianGrid strokeDasharray="3 3" stroke="#B7C5D3" />
                <XAxis dataKey={yearDistributionData.length ? 'name' : 'week'} stroke="#51606F" tick={{ fontSize: 12 }} />
                <YAxis stroke="#51606F" tick={{ fontSize: 12 }} />
                <Tooltip />
                <Line type="monotone" dataKey={yearDistributionData.length ? 'total' : 'responses'} stroke="#2ECC71" strokeWidth={2.5} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </ModuleBlock>
      ...
      <ModuleBlock componentId="MOD-A9" eyebrow="Signal Review" title="Operational Insights">
        <div className={styles.insightList}>
          ...
          <div className={styles.insightRow}>
            ...
            <p className={styles.insightText}>This page now uses student-profile to pull the current roster for {selectedGroup.code} when the backend is available.</p>
          </div>
          <div className={styles.insightRow}>
            <SystemTag hazard>Next integration</SystemTag>
            <p className={styles.insightText}>Team membership is still mock-backed. The next backend slice should connect team-formation and team persistence so roster and team assignments come from the same source.</p>
          </div>
        </div>
      </ModuleBlock>
    </div>
